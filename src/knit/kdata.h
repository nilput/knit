...
#include "../jadwal/third_party/strhash/superfasthash.h" //in jadwal dirs

/*hashtable defs*/
    typedef struct knit_str    knit_vars_jadwal_key_type;   //internal notes: there is no indirection, init/deinit must be used on pair objects

    typedef struct knit_obj  * knit_vars_jadwal_value_type; //                there is indirection,    new/destroy must be used
...
#include "knit_vars_jadwal.h" //autogenerated jadwal.h and prefixed by vars_

/*hashtable defs*/
//TODO, have a single convention for eq funcs return meaning
    typedef struct knit_obj *kobj_jadwal_key_type; 
    typedef struct knit_obj *kobj_jadwal_value_type; 
...
/*end of hashtable defs*/
#define KOBJ_JADWAL_DATA_ARG
#include "kobj_jadwal.h"  //autogenerated jadwal.h and prefixed by kobj_

struct knit_dict {
    KNIT_OBJ_HEAD;
    struct kobj_jadwal ht;
};
struct knit_int {
    KNIT_OBJ_HEAD;
    int value;
...
struct knit_exec_state {
    struct knit_vars_jadwal global_ht;
